import { CARD_DEFINITION_IDS } from './cards'
import type { CardColor, CardDefinitionId } from './types'

const CARD_ID = CARD_DEFINITION_IDS

export const THEME_DECK_IDS = {
  RED_BLUE_SKIRMISH: '17937130-b84a-45da-a9a2-1d5179ea2303',
  BLUE_GREEN_INTERCEPT: '4c513ec7-edc2-4a5a-b965-52a770260cc7',
  GREEN_RED_FRONTLINE: '6a6d5a62-6c24-4a09-b9b7-ae4dea011be6',
  RED_TOTAL_ASSAULT: '060d14bb-5e7c-4150-ab19-11168b56c622',
  BLUE_MOBILE_INTERCEPT: '7a07f81f-d165-493a-ae8f-92f66b8a1c5a',
  GREEN_FORTRESS_CYCLE: '61d4a505-c40a-47a9-b012-83d4dbe54689',
} as const

export type ThemeDeckId = (typeof THEME_DECK_IDS)[keyof typeof THEME_DECK_IDS]

export type ThemeDeck = {
  id: ThemeDeckId
  name: string
  colors: readonly [CardColor, ...CardColor[]]
  description: string
  cardDefinitionIds: readonly CardDefinitionId[]
  tournamentShuffleSalt: number
}

// Source order is stable because fixed-seed tournament shuffling starts from this order.
const RED_BLUE_SKIRMISH = [
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.FIREBALL_ASSAULT,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.RETURN_FIRE,
  CARD_ID.FIREBALL_ASSAULT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.LIFE_DROPLET,
] satisfies CardDefinitionId[]

const BLUE_GREEN_INTERCEPT = [
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TRANSFER,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.LIFE_DROPLET,
  CARD_ID.TRANSFER,
  CARD_ID.TRANSFER,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.GEODE_MINER,
  CARD_ID.GEODE_MINER,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.ABUNDANCE,
] satisfies CardDefinitionId[]

const GREEN_RED_FRONTLINE = [
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.GEODE_MINER,
  CARD_ID.GEODE_MINER,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.ABUNDANCE,
  CARD_ID.GEODE_MINER,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.LEYLINE_MINING_GIANT,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.ABUNDANCE,
  CARD_ID.LIFE_CYCLE,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.RETURN_FIRE,
] satisfies CardDefinitionId[]

const RED_TOTAL_ASSAULT = [
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.SPARK_SWORDSMAN,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.BURNING_VANGUARD,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.SOLITARY_PEAK_SWORDSMAN,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.FORMATION_CLEARING_MERCENARY,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.CRIMSON_BLADE_INFILTRATOR,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.BEACON_HEAVY_CAVALRY,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.LONE_ARMY_GENERAL,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.ASH_DISMANTLER,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.EXHAUSTED_VOLCANO_DRAGON,
  CARD_ID.RETURN_FIRE,
  CARD_ID.RETURN_FIRE,
  CARD_ID.RETURN_FIRE,
  CARD_ID.FIREBALL_ASSAULT,
  CARD_ID.FIREBALL_ASSAULT,
] satisfies CardDefinitionId[]

const BLUE_MOBILE_INTERCEPT = [
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.TIDEWAY_SCOUT,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SPRAY_HERALD,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.SURGING_DUELIST,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.MIST_RETURNING_MESSENGER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.TIDEFRONT_FORTIFIER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.AZURE_WAVE_VOYAGER,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.DEEP_TIDE_INTERCEPTOR,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.WAVE_RETURN_MAGE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.EPHEMERAL_DEEP_WHALE,
  CARD_ID.LIFE_DROPLET,
  CARD_ID.LIFE_DROPLET,
  CARD_ID.TRANSFER,
  CARD_ID.TRANSFER,
  CARD_ID.TRANSFER,
] satisfies CardDefinitionId[]

const GREEN_FORTRESS_CYCLE = [
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.OAKBARK_SENTINEL,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.ROOTED_ANCIENT,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.VINE_SNARE_HUNTER,
  CARD_ID.GEODE_MINER,
  CARD_ID.GEODE_MINER,
  CARD_ID.GEODE_MINER,
  CARD_ID.GEODE_MINER,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.ROOT_FORT_REARGUARD,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.GREAT_TREE_GUARDIAN,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.FOREST_CAGE_BEASTMASTER,
  CARD_ID.LEYLINE_MINING_GIANT,
  CARD_ID.LEYLINE_MINING_GIANT,
  CARD_ID.LEYLINE_MINING_GIANT,
  CARD_ID.LEYLINE_MINING_GIANT,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.DREAMWALKING_FOREST_GIANT,
  CARD_ID.ABUNDANCE,
  CARD_ID.ABUNDANCE,
  CARD_ID.ABUNDANCE,
  CARD_ID.LIFE_CYCLE,
  CARD_ID.LIFE_CYCLE,
] satisfies CardDefinitionId[]

export const THEME_DECKS = [
  {
    id: THEME_DECK_IDS.RED_TOTAL_ASSAULT,
    name: 'ブンブンぶん殴り',
    colors: ['red'],
    description: '赤の高攻撃・一騎当千・直接火力を集め、複数地点から突破する。',
    cardDefinitionIds: RED_TOTAL_ASSAULT,
    tournamentShuffleSalt: 4_146_351_749,
  },
  {
    id: THEME_DECK_IDS.GREEN_FORTRESS_CYCLE,
    name: 'じゃぶじゃぶマネー',
    colors: ['green'],
    description: '緑の高防御・採掘・捕獲を維持し、資源を再展開へつなげる。',
    cardDefinitionIds: GREEN_FORTRESS_CYCLE,
    tournamentShuffleSalt: 1_477_358_106,
  },
  {
    id: THEME_DECK_IDS.BLUE_MOBILE_INTERCEPT,
    name: '急襲レールウェイ',
    colors: ['blue'],
    description: '青の高進軍・反撃・招集・テレポートで戦線を組み替える。',
    cardDefinitionIds: BLUE_MOBILE_INTERCEPT,
    tournamentShuffleSalt: 2_168_962_053,
  },
  {
    id: THEME_DECK_IDS.RED_BLUE_SKIRMISH,
    name: '爆弾魔',
    colors: ['red', 'blue'],
    description: '青の進軍・橋頭堡・招集で侵入路を作り、赤の単独突破を狙う。',
    cardDefinitionIds: RED_BLUE_SKIRMISH,
    tournamentShuffleSalt: 3_350_137_494,
  },
  {
    id: THEME_DECK_IDS.GREEN_RED_FRONTLINE,
    name: 'メガトンハンマー',
    colors: ['green', 'red'],
    description: '緑の採掘拠点を維持し、赤の基礎火力・撤去・刺客で反攻する。',
    cardDefinitionIds: GREEN_RED_FRONTLINE,
    tournamentShuffleSalt: 63_853_786,
  },
  {
    id: THEME_DECK_IDS.BLUE_GREEN_INTERCEPT,
    name: '真綿',
    colors: ['blue', 'green'],
    description: '青の位置制御と反撃を、緑の高防御・捕獲・しんがりで支える。',
    cardDefinitionIds: BLUE_GREEN_INTERCEPT,
    tournamentShuffleSalt: 2_589_017_928,
  },
] satisfies ThemeDeck[]

export const THEME_DECK_BY_ID = {
  [THEME_DECK_IDS.RED_TOTAL_ASSAULT]: THEME_DECKS[0],
  [THEME_DECK_IDS.GREEN_FORTRESS_CYCLE]: THEME_DECKS[1],
  [THEME_DECK_IDS.BLUE_MOBILE_INTERCEPT]: THEME_DECKS[2],
  [THEME_DECK_IDS.RED_BLUE_SKIRMISH]: THEME_DECKS[3],
  [THEME_DECK_IDS.GREEN_RED_FRONTLINE]: THEME_DECKS[4],
  [THEME_DECK_IDS.BLUE_GREEN_INTERCEPT]: THEME_DECKS[5],
} satisfies Record<ThemeDeckId, ThemeDeck>
